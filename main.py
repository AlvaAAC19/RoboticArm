...
class MainScreen(Screen):
    ...
    def isBallOnTallTower(self):
        while self.is_BallOnTallTower:
            if (cyprus.read_gpio() & 0b0010 == 0):  # binary bitwise AND of the value returned from read.gpio()
                sleep(1)
                if (cyprus.read_gpio() & 0b0001 == 0):  # a little debounce logic
                    print("GPIO on port P7 is activated")
            else:
                print("GPIO on port P7 is not detecting")
                sleep(1)
